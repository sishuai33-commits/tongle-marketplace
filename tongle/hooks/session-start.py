...
from lib import paths, health, manifest, state

# Windows Git Bash 兼容（bug1：子进程 python3 继承）
...
def main():
    ...
    _run_quiet(["python3", daemon, "access-update"])
    ...
    # §1 patterns.yaml
    active = manifest.parse_patterns(instincts_dir)
    manual_instincts = '\n'.join(manifest.instinct_line(x) for x in active)
    manual_count = len(active)

    # §2 Working Memory
    wm_section, wm_count = manifest.parse_working_memory(cc_memory_dir)

    # §3 active-context.md
    manifest.gen_active_context(instincts_dir, session_id, manual_instincts, manual_count, wm_section)
    ...
    # §6.4 Wiki 资产路由（静默注入）
    asset_manifest = _run_quiet(["python3", os.path.join(hooks_dir, "build-asset-manifest.py")]).strip()

    # §6.5 维护门禁（调 lib/health.maintenance_check，子进程隔离 sys.exit；读 marker）
    _run_quiet(["python3", "-c",
                f"import sys; sys.path.insert(0, {_PLUGIN_ROOT!r}); "
                f"from lib import health; health.maintenance_check()"])
    maint_section = ""
