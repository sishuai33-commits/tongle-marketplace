#!/usr/bin/env python3
"""
source-scanner.py — 采集环主动扫描器（三源）

架构范式 §四采集环：ke 主动从信息源（本地文件/对话历史/IMA知识库）采集原料。
observe.sh 是被动记工具行为（Read/Write/Edit/Search），本扫描器是**主动扫信息源**，
补 observe 单源缺口（任务#3①采集环改进，2026-07-02）。

守原则3"判别复用同库不另造"：scanner 只产原料（source-observations.jsonl），
**不判关系**（关系判别归 collector，scanner 不重复造判别器）。
守红线②：source-observations 随数据长不强建 schema（plain jsonl 追加）。
守红线⑥：只记文件变更信号**不总结内容**（防"漂亮总结"伪洞察，内容摘录留人裁看原文件）。
守简单方案优先：源1 只扫 .md + mtime 游标，不算 hash/不语义解析。

三源（本任务顺序推进，源3 未实现时 --source 缺省只跑源1+源2）：
  源1 local_file：扫项目根 .md 文件 mtime 增量 → 文件变更信号
  源2 transcript：扫对话历史 transcript 提取决策动作信号（决策/修正），补 observe 只记工具
    不记语义的缺口——决策/拍板/推翻/纠偏是判别经验库高价值原料，observe 记 Read/Write
    不记"为什么读/读出什么认知"。SessionEnd 同步扫（实测 1.3MB transcript 解析+正则 0.007s，
    1.5s timeout 内绰绰有余，无需异步 worker——异步是 LLM-judge 慢工作的方案，正则提取不需要）
  源3 ima：IMA 知识库 crawler 接入（已有 crawler，接入 collector 判别需设计，见待办）

触发：
  - SessionEnd：session-end-discriminate.sh 串联 source-scanner(源1+源2) + collector --source
  - 慢环全量：python3 hooks/source-scanner.py --full-scan
  - 手动：python3 hooks/source-scanner.py [--source local_file|transcript|all] [--full-scan]
         源2 手动/慢环需 --transcript <path> --session <id>（SessionEnd 自动从 stdin 解析传入）

输入：本地文件系统（源1）/ transcript 文件（源2）/ crawler（源3，待办）
游标：~/.claude/instincts/.source-scan-cursor-local（源1 last scan max mtime，浮点 epoch 秒，避免 ISO 截断精度导致重复采）
      源2 无 mtime 游标（会话级一次性，靠去重键 session+action+context 防重复采）
输出：~/.claude/instincts/source-observations.jsonl
  源1 每条 = {ts, source, signal_type, path, mtime, size}
  源2 每条 = {ts, source, signal_type, session, action_type, context, transcript_path}
  fail-open：扫不到也 exit 0（不阻断 SessionEnd/慢环）

关系候选模式（留给 collector --source 模式判，本脚本不判）：
  - file_change_candidate：项目根下 .md 文件 mtime > 游标 = 潜在新知识/演进信号
    （文件变更 vs 现有 wiki 记录的关系留人裁，scanner 只标"有变更"）
  - transcript_candidate：transcript 含决策动作 = 潜在认知演进/判别经验原料
    （决策/修正动作的认知关系留人裁，scanner 只标"有动作"）
...
SOURCE_OBS = os.path.join(INSTINCTS_DIR, "source-observations.jsonl")
...
DEFAULT_SOURCE_ROOT = os.path.join(_home, "Documents", "My_Code_Projects")
...
EVOLVE_WINDOW_NOTE = "文件变更信号 → collector --source 判 file_change_candidate → 人裁关系"
...
IMA_CRED_DIR = os.environ.get("IMA_CRED_DIR", os.path.join(_home, ".config", "ima"))
...
def load_cursor(path):
    """上次扫描到的 max mtime（浮点 epoch 秒）。增量跳过 mtime <= cursor 的文件。

    用浮点 epoch 而非 ISO：ISO 截断到秒会丢亚秒精度，导致 mtime 恰在秒内的文件
    每次增量重复采（04:01:02.7 > 04:01:02.0 cursor）。浮点 epoch 是内部状态非数据，
    格式自由，mtime 字段仍用 ISO 展示。
    """
...
def save_cursor(path, mtime_epoch):
    try:
        with open(path, "w", encoding="utf-8") as f:
            f.write(str(mtime_epoch))
    except OSError:
        pass  # 游标丢失不影响本次输出（下次全扫，可接受）


def append_source_obs(entry):
    """棘轮只升：append（与 observations.jsonl 同形态，collector 统一消费）。"""
    try:
...
def scan_local_file(root, full_scan):
    ...
    """
    cursor_epoch = None if full_scan else load_cursor(CURSOR_LOCAL)
    ...
    if max_mtime_epoch is not None:
        save_cursor(CURSOR_LOCAL, max_mtime_epoch)
...
def scan_transcript(transcript_path, session_id):
    ...
    observe.sh 记工具行为（Read/Write/Search）不记语义——决策/拍板/推翻/纠偏这类
...
def scan_ima(full_scan):
    ...
    # 游标：max modify_time（毫秒 epoch 字符串）。首次/全量=None
    cursor_mtime = None
    if not full_scan and os.path.exists(CURSOR_IMA):
        try:
            cursor_mtime = open(CURSOR_IMA).read().strip() or None
        except OSError:
            cursor_mtime = None
...
def main():
    ...
    ap.add_argument("--source", choices=["local_file", "transcript", "ima", "all"],
                    default="all", help="扫描哪个源（all=已实现的都跑，默认 local_file）")
